...
TestVersionString=`grep -E 'version.test.*=' DCBModule_Category.podspec`
TestVersionNumber=`tr -cd 0-9 <<<"$TestVersionString"`
NewTestVersionNumber=$TestVersionNumber

GrayVersionString=`grep -E 'version.gray.*=' DCBModule_Category.podspec`
...
sed -i "" "${DevelopVersionLineNumber}s/${DevelopVersionNumber}/${NewDevelopVersionNumber}/g" DCBModule_Category.podspec
...
TestVersionLineNumber=`grep -nE 'version.test.*=' DCBModule_Category.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" DCBModule_Category.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' DCBModule_Category.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" DCBModule_Category.podspec
echo "current gray version is ${GrayVersionNumber}, new version is ${NewGrayVersionNumber}, line number is ${GrayVersionLineNumber}"

VersionString=`grep -E 's.version.*=' DCBModule_Category.podspec`
...
VersionLineNumber=`grep -nE 's.version.*=' DCBModule_Category.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" DCBModule_Category.podspec
...
pod repo push ${Repo} DCBModule_Category.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
