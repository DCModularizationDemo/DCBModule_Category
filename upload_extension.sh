...
DevelopVersionString=`grep -E 'version.develop.*=' DCBModule_Category_Extension.podspec`
DevelopVersionNumber=`tr -cd 0-9 <<<"$DevelopVersionString"`
NewDevelopVersionNumber=$DevelopVersionNumber

TestVersionString=`grep -E 'version.test.*=' DCBModule_Category_Extension.podspec`
TestVersionNumber=`tr -cd 0-9 <<<"$TestVersionString"`
NewTestVersionNumber=$TestVersionNumber

GrayVersionString=`grep -E 'version.gray.*=' DCBModule_Category_Extension.podspec`
...
DevelopVersionLineNumber=`grep -nE 'version.develop.*=' DCBModule_Category_Extension.podspec | cut -d : -f1`
sed -i "" "${DevelopVersionLineNumber}s/${DevelopVersionNumber}/${NewDevelopVersionNumber}/g" DCBModule_Category_Extension.podspec
...
TestVersionLineNumber=`grep -nE 'version.test.*=' DCBModule_Category_Extension.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" DCBModule_Category_Extension.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' DCBModule_Category_Extension.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" DCBModule_Category_Extension.podspec
echo "current gray version is ${GrayVersionNumber}, new version is ${NewGrayVersionNumber}, line number is ${GrayVersionLineNumber}"

VersionString=`grep -E 's.version.*=' DCBModule_Category_Extension.podspec`
...
VersionLineNumber=`grep -nE 's.version.*=' DCBModule_Category_Extension.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" DCBModule_Category_Extension.podspec
...
pod repo push ${Repo} DCBModule_Category_Extension.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
